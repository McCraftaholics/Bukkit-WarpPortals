...
public class CmdPortalCreate extends CommandHandlerObject {
	...
	public static boolean handle(Player sender, String[] args, CommandHandler main) {
		if (sender.hasPermission("warpportals.admin.portal.create")) {
			if (args.length == 3) {
				try {
						...
						if (main.mPortalManager.getPortalInfo(args[0].trim()) == null) {
							try {
								...
								if (tpCoords != null) {
									...
									if (blockType != null) {
										/*
										 * Test to see if it is a valid block
										 * type (not a fishing rod for example)
										 */
										if (blockType.isBlock()) {
											/*
											 * Test to see if the block is
											 * solid, recommend to the player
											...
											/*
											 * Test if curItem is a tool or
											 * other non-block item
											 */
